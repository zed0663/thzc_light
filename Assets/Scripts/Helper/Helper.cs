...
public static class Helper
{
    ...
    public static bool IsEqualOrSmaller (double valueBigger, int unitBigger, double valueSmaller, int unitSmaller)
    {
        ...
        //return true;
        Helper.FixUnit(ref valueBigger, ref unitBigger);
        Helper.FixUnit(ref valueSmaller, ref unitSmaller);

        if (unitBigger < unitSmaller)
        {
            // Debug.LogError("价格对比失败");
            return false;
        }

        double valueBigger1 = valueBigger;
        int unitBigger1 = unitBigger;



        double valueSmaller1 = valueSmaller;
        int unitSmaller1 = unitSmaller;

        if (unitBigger1 != unitSmaller1)
        {
            if (Mathf.Abs(unitBigger1 - unitSmaller1) <= 5)
            {
                return valueSmaller1 <= (valueBigger1 * Math.Pow(1000, unitBigger - unitSmaller));
            }
            else
            {
                return true;
        ...
        {
            return valueBigger1 >= valueSmaller1;
        }
    }
    ...
    public static double GetCarPrice(int level, ref double amount, ref int amount_unit)
    {
        var data = GameData.Instance.ItemDataGroup.GetDataItemWithLevel(level);
        int numberbuytimes = PlayerData.GetNumberBuyItemProfitCoefficient(level);
        double _price = data.Prices;
        _price = _price * Math.Pow(data.PricesCoefficient, numberbuytimes);
        amount = _price;
        amount_unit = data.PricesUnit;
        FixUnit(ref amount, ref amount_unit);
