...
namespace Ez.Pooly
{
    ...
    public class PoolySpawnerEditor : QEditor
    {
        ...
        protected override void GenerateInfoMessages()
        {
            ...
            infoMessage.Add("AutoStartNever", new InfoMessage() { title = "Auto Start Never", message = "The spawner will not automatically start spawning.\nIt's your responsability to start the spawning cycle for this spawner.", show = new AnimBool(false, Repaint), type = InfoMessageType.Help });
            infoMessage.Add("SpawnForever", new InfoMessage() { title = "Spawn Forever", message = "The spawner will spawn prefabs forever and it will stop only when you call either StopSpawn or PauseSpawn methods.\nIt's your responsability to stop or pause the spawning cycle for this spawner.", show = new AnimBool(false, Repaint), type = InfoMessageType.Help });
            infoMessage.Add("NoSpawnPoint", new InfoMessage() { title = "", message = "Set at least one spawn point by referencing a Transform from the scene or set Use Spawner As Spawn Location as true.", show = new AnimBool(false, Repaint), type = InfoMessageType.Info });
            infoMessage.Add("NoPrefabs", new InfoMessage() { title = "SPAWNER IS DISABLED", message = "Reference at least one prefab.", show = new AnimBool(false, Repaint), type = InfoMessageType.Error });
        }
                ...
                QUI.BeginVertical();
                {
                    ...
                    if(QUI.BeginFadeGroup(animBoolShowSpawnLocationsOptions.faded))
                    {
                        ...
                        QUI.BeginHorizontal(WIDTH_420);
                        {
                            ...
                            tempLinesColor = QUI.ColorField(tempLinesColor, false, false, true, null, 20);
                            if(QUI.EndChangeCheck())
                            {
                                savePoolySettings = true;
