	...
	public class SpeedHackDetector : ACTkDetectorBase<SpeedHackDetector>
	{
		...
		private void Update()
		{
			...
			if (cheatedEnvironment || cheatedRealtime)
			{
				...
				{
#if ACTK_DEBUG_ENABLED
					Debug.LogWarning(FinalLogPrefix + "detection! Allowed false positives left: " + (maxFalsePositives - currentFalsePositives), this);
#endif
			...
			{
#if ACTK_DEBUG_ENABLED
				Debug.Log(FinalLogPrefix + "success shot! Shots till cool down: " + (coolDown - currentCooldownShots), this);
#endif
				currentCooldownShots++;
				if (currentCooldownShots >= coolDown)
				{
#if ACTK_DEBUG_ENABLED
					Debug.Log(FinalLogPrefix + "cool down!", this);
#endif
