...
#if (NET_4_6 || NET_STANDARD_2_0) && !(UNITY_WEBGL || UNITY_WEBGL_API)
...
namespace CodeStage.AntiCheat.Detectors
{
	...
	public class TimeCheatingDetector : ACTkDetectorBase<TimeCheatingDetector>
	{
		...
		{
			/// <summary>
			/// Indicates about success of the operation.
			/// </summary>
			public bool success;
		...
		public static async Task<OnlineTimeResult> GetOnlineTimeTask(string url, RequestMethod method = RequestMethod.Head)
		{
			var uri = UrlToUri(url);
			return await GetOnlineTimeTask(uri, method);
		}
		...
		private static UnityWebRequest GetWebRequest(Uri uri, RequestMethod method)
		{
...
#else
			var request = new UnityWebRequest(uri.AbsoluteUri, method == RequestMethod.Head ? UnityWebRequest.kHttpVerbHEAD : UnityWebRequest.kHttpVerbGET)
#endif
			{
			...
			if (method == RequestMethod.Head)
			{
				...
				{
					Debug.LogError(FinalLogPrefix + "Couldn't get SDK version or set Accept-Encoding header.");
				}
			}
#endif
...
#endif
			Uri result;
			var success = Uri.TryCreate(url, UriKind.Absolute, out result);
			if (!success)
			{
				Debug.LogError(FinalLogPrefix + "Could not create URI from URL: " + url);
			}

			return result;
		}

		// naive but blazingly fast parsing of standard HTTP GMT Date
		// ex. 'Tue, 18 Sep 2018 16:28:26 GMT' (ddd, dd MMM yyyy HH:mm:ss 'GMT')
		// (C) focus / codestage.net
		private static bool TryGetDate(string source, out DateTime date)
		{
			...
			{
				Debug.LogError(FinalLogPrefix + "Error while parsing date: " + e);
		...
		public bool ForceCheck()
		{
			...
			{
				Debug.LogWarning(FinalLogPrefix + "Detector should be started to use ForceCheck().");
			...
			{
				Debug.LogWarning(FinalLogPrefix + "Can't force cheating check since another check is already in progress.");
		...
		public IEnumerator ForceCheckEnumerator()
		{
			if (!started || !isRunning)
			{
				Debug.LogWarning(FinalLogPrefix + "Detector should be started to use ForceCheckEnumerator().");
				LastError = ErrorKind.NotStarted;
				LastResult = CheckResult.Error;
				yield break;
			}
		...
		public async Task<CheckResult> ForceCheckTask()
		{
			if (!started || !isRunning)
			{
				Debug.LogWarning(FinalLogPrefix + "Detector should be started to use ForceCheckTask().");
				LastError = ErrorKind.NotStarted;
				LastResult = CheckResult.Error;
				return LastResult;
			}
		...
		{
			return base.DetectorHasCallbacks() || CheatChecked != null || cheatChecked != null;
		}
		...
		private IEnumerator CheckForCheat()
		{
			...
			if (lastOfflineOnlineDifference != 0)
			{
				...
				{
					Debug.LogWarning(FinalLogPrefix + "Please consider increasing realCheatThreshold to reduce false positives chance!");
				}
...
#pragma warning restore 67
		...
		#endregion
#else
		private const string ErrorMessage = FinalLogPrefix + " is disabled with ACTK_PREVENT_INTERNET_PERMISSION conditional or is not supported on current platform!";
		protected override void StartDetectionAutomatically()
		{
