	...
	public class WallHackDetector : ACTkDetectorBase<WallHackDetector>
	{
		...
		private WallHackDetector StartDetectionInternal(Action callback, Vector3 servicePosition, byte falsePositivesInRow)
		{
			...
			if (checkRaycast || checkController || checkRigidbody)
			{
				...
				{
					Debug.LogError(FinalLogPrefix + "IgnoreRaycast physics layer should collide with itself to avoid false positives! See readme's troubleshooting section for details.");
				}
			}
#endif
			...
			if (enabled && gameObject.activeSelf)
			{
				...
				if (checkWireframe && wfCamera == null)
				{
					...
					else
					{
						...
						{
							Debug.LogWarning(FinalLogPrefix + "can't detect wireframe cheats on this platform due to lack of needed shader support!", this);
							checkWireframe = false;
						}
						else
						{
		...
		private void StartRigidModule()
		{
			...
			if (rigidPlayer.transform.localPosition.z <= 1f && rigidbodyDetections > 0)
			{
				...
				Debug.Log(FinalLogPrefix + "rigidbody success shot! False positives counter reset.", this);
#endif
				rigidbodyDetections = 0;
			}
		...
		private void StartControllerModule()
		{
			...
			{
#if ACTK_DEBUG_ENABLED
				Debug.Log(FinalLogPrefix + "controller success shot! False positives counter reset.", this);
#endif
				controllerDetections = 0;
			}
		...
		private IEnumerator CaptureFrame()
		{
			...
			if (!detected)
			{
				...
				{
#if ACTK_DEBUG_ENABLED
					Debug.Log(FinalLogPrefix + "wireframe success shot! False positives counter reset.", this);
#endif
					wireframeDetections = 0;
				}
			}
			else
			{
				...
				Debug.LogWarning(FinalLogPrefix + "wireframe wallhack detected. Details below:\n" +
		...
		private void ShootRaycastModule()
		{
			if (Physics.RaycastNonAlloc(serviceContainer.transform.position, serviceContainer.transform.TransformDirection(Vector3.forward), rayHits, 1.5f, raycastMask) > 0)
			{
				...
				{
#if ACTK_DEBUG_ENABLED
					Debug.Log(FinalLogPrefix + "raycast success shot! False positives counter reset.", this);
#endif
					raycastDetections = 0;
				}
			}
		...
		private bool Detect()
		{
			...
			{
#if ACTK_DEBUG_ENABLED
				Debug.LogWarning(FinalLogPrefix + "final detection!", this);
#endif

#if ACTK_DETECTION_BACKLOGS
				Debug.LogWarning(FinalLogPrefix +
			...
			{
				Debug.LogWarning(FinalLogPrefix + "detection!", this);
			}
#endif
